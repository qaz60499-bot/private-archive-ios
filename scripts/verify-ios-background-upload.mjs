...
assert(source.includes('private let maxActiveUploadPipelines = 3'), 'native background uploads must keep a shallow durable pipeline instead of flooding iOS with the whole batch')
assert(source.includes('private func activeUploadPipelineCountLocked'), 'reserve and content scheduling must share one bounded pipeline counter')
assert(contentSchedule.includes('activeUploadPipelineCountLocked(excluding: record.id) < maxActiveUploadPipelines'), 'content scheduling must enforce the bounded pipeline before creating a task')
assert(contentSchedule.includes('value.status = "waiting"') && contentSchedule.includes('if let queued { notify(queued) }'), 'no-slot content records must remain visibly queued rather than consuming retry budget')
...
assert(contentSchedule.indexOf('guard let updated else') < contentSchedule.indexOf('contentUploadSession().uploadTask'), 'no-slot records must stay durably queued without creating extra background tasks')
...
assert(reconcile.includes('.sorted { $0.createdAt < $1.createdAt }'), 'bounded pipeline refill must be FIFO rather than starving older jobs')
assert(reconcile.includes('try self.scheduleContent(record, earliest: nil)') && reconcile.includes('try self.scheduleReserve(record, earliest: nil)'), 'ordinary queued jobs must refill slots without being treated as retries')
