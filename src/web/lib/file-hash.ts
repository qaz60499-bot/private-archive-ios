...
class IncrementalSha256 {
  ...
  digest(): Uint8Array {
    ...
    for (let index = 0; index < this.state.length; index += 1) {
      ...
      const base = index * 4
      output[base] = value >>> 24
      output[base + 1] = value >>> 16
      output[base + 2] = value >>> 8
      output[base + 3] = value
    }
    return output
  }
...
export async function sha256File(file: Blob): Promise<string | undefined> {
  try {
    if (file.size <= NATIVE_DIGEST_MAX_BYTES && globalThis.crypto?.subtle) {
    ...
    for (let offset = 0; offset < file.size; offset += STREAM_CHUNK_BYTES) {
      const bytes = new Uint8Array(await file.slice(offset, Math.min(file.size, offset + STREAM_CHUNK_BYTES)).arrayBuffer())
      hasher.update(bytes)
      chunksSinceYield += 1
