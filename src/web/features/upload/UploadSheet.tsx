...
export function UploadSheet() {
  const { uploadOpen, setUploadOpen, online, importStatus, runImport, runNativePhotoImport } = useArchive()
  ...
  const submitMobileFiles = (selected: File[]) => {
    if (!selected.length) return
    // Commit the sheet close before any durable file work starts. On WKWebView the old
    // same-turn close + IndexedDB/native staging path could leave the modal visually
    // pinned over the app while the selected Files were being materialized.
    setUploadOpen(false)
    requestAnimationFrame(() => globalThis.setTimeout(() => { void addFiles(selected) }, 0))
  }
  const nativePhotoPicker = nativeIos && canUseIosBackgroundUpload(storageBackend)
